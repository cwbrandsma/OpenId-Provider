...
using UnityMG.UnityMembershipProvider;

namespace UnityMG.OpenId.Provider.Controllers
{
        ...
        public ActionResult SendAssertion()
        {
                ...
                (anonReq != null && anonReq.IsApproved.Value))
            {
                ...
                if (claimsRequest != null)
                {
                    ...
                    var dbo = new UserRepository("",ConfigurationManager.ConnectionStrings["ApplicationServices"].ConnectionString, null);
                    var user = dbo.GetUser(User.Identity.Name);
                    ...
                    {
                        claimsResponse.FullName = user.UserName;
                    }

                    if (claimsRequest.TimeZone != DemandLevel.NoRequest)
                    {
